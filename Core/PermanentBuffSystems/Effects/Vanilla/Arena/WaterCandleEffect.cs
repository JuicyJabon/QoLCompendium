namespace QoLCompendium.Core.PermanentBuffSystems.Effects.Vanilla.Arena
{
    public class WaterCandleEffect : IPermanentBuff
    {
        ...
        {
            if (!player.Player.buffImmune[BuffID.WaterCandle] && !player.Player.GetModPlayer<PermanentBuffPlayer>().PermanentBuffsBools[(int)PermanentBuffPlayer.PermanentBuffs.WaterCandle])
            {
