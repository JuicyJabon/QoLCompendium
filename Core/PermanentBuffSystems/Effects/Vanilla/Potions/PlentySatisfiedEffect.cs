namespace QoLCompendium.Core.PermanentBuffSystems.Effects.Vanilla.Potions
{
    public class PlentySatisfiedEffect : IPermanentBuff
    {
        ...
        {
            if (!player.Player.buffImmune[BuffID.WellFed2] && !player.Player.GetModPlayer<PermanentBuffPlayer>().PermanentBuffsBools[(int)PermanentBuffPlayer.PermanentBuffs.PlentySatisfied])
            {
