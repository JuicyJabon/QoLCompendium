...
namespace QoLCompendium.Core.Changes
{
    public class AlwaysEventSystem : ModSystem
    {
        ...
        {
            Player p = Main.LocalPlayer;
            if (p.GetQoLCPlayer().sunPedestal == true)
            {
                if (!Main.dayTime)
                {
                    Main.dayTime = true;
            ...
            if (p.GetQoLCPlayer().moonPedestal == true)
            {
                if (Main.dayTime)
                {
                    Main.dayTime = false;
            ...
            if (!Main.dayTime && p.GetQoLCPlayer().bloodMoonPedestal == true)
            {
                Main.bloodMoon = true;
            }

            if (Main.dayTime && p.GetQoLCPlayer().eclipsePedestal == true)
            {
                Main.eclipse = true;
            }
        }
    ...
    public class DisableEvilSpreadSystem : ModSystem
    {
        ...
        {
            CorruptionSpreadDisabled = QoLCompendium.mainServerConfig.DisableEvilBiomeSpread;
        }
...
#pragma warning restore CA2211
    ...
    public class DisableMeteors : ModSystem
    {
        ...
        {
            if (!QoLCompendium.mainServerConfig.NoMeteorSpawns)
            {
                orig();
                return;
            }
    ...
    public class SeasonalActivation : ModSystem
    {
        ...
        {
            if (QoLCompendium.mainServerConfig.HalloweenActive)
                Main.halloween = true;
            if (QoLCompendium.mainServerConfig.ChristmasActive)
                Main.xMas = true;
        }
    }
    ...
    public class DisableCredits : ModSystem
    {
        public override void Load()
        {
            if (QoLCompendium.mainServerConfig.DisableCredits)
            {
    ...
    public class StarFallSystem : ModSystem
    {
        public override void PostUpdateWorld()
        {
            Star.starfallBoost = QoLCompendium.mainServerConfig.MoreFallenStars;
        }
    }
}
