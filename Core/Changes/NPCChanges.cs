...
namespace QoLCompendium.Core.Changes
{
    public class InvincibleTownies : GlobalNPC
    {
        ...
        public override void SetDefaults(NPC npc)
        {
            ...
            if (npc.type == NPCID.DD2EterniaCrystal && QoLCompendium.mainServerConfig.TownNPCsDontDie)
            {
                npc.dontTakeDamageFromHostiles = false;
                return;
            }
            npc.dontTakeDamageFromHostiles = dontTakeDamageFromHostiles;
            if (npc.friendly && QoLCompendium.mainServerConfig.TownNPCsDontDie)
            {
    ...
    public class FastTownieSpawn : ModSystem
    {
        public override void PreUpdateWorld()
        {
            ...
            if (player.active)
            {
                if (!player.dead && QoLCompendium.mainServerConfig.FastTownNPCSpawns)
                {
                    rate = true;
                }
            }
    ...
    public class TownieSpawns : ModSystem
    {
        public override void PreUpdateWorld()
        {
            if (QoLCompendium.mainServerConfig.TownNPCSpawnImprovements)
            {
    ...
    public class LiveInEvil : ModSystem
    {
        ...
        private void LiveInCorrupt(ILContext il)
        {
            ...
            if (!c.TryGotoNext(
                ...
                return;
            c.EmitDelegate<Func<int, int>>((returnValue) => QoLCompendium.mainServerConfig.TownNPCsLiveInEvil ? 114514 : returnValue);
        }
    ...
    public class HappinessOverride : ModSystem
    {
        ...
        public override void Load()
        {
            ...
            IL_Main.DrawNPCChatButtons += il =>
            {
                ...
                c.EmitDelegate((string text) => !QoLCompendium.mainServerConfig.DisableHappiness ? text : "");
            };
        }
        ...
        {
            return orig(self) || QoLCompendium.mainServerConfig.OverridePylonSales;
        }
    }
    ...
    public class DisableNaturalBossSpawns : ModSystem
    {
        ...
        {
            if (QoLCompendium.mainServerConfig.NoNaturalBossSpawns)
            {
    ...
    public class SpawnRateEdits : GlobalNPC
    {
        ...
        {
            if (AnyBossAlive() && player.Distance(Main.npc[boss].Center) < 6000f && QoLCompendium.mainServerConfig.NoSpawnsDuringBosses)
            {
                maxSpawns = 0;
            }
            if (player.GetQoLCPlayer().enemyEraser)
            {
                maxSpawns = 0;
            }
            if (player.GetQoLCPlayer().enemyAggressor)
            {
                spawnRate = (int)(spawnRate * 0.1);
                maxSpawns = (int)(maxSpawns * 10f);
            }
            if (player.GetQoLCPlayer().enemyCalmer)
            {
...
#pragma warning restore CA2211
    ...
    public class TowerShieldHealth : ModSystem
    {
        public override void PreUpdateWorld()
        {
            if (NPC.ShieldStrengthTowerVortex > QoLCompendium.mainServerConfig.LunarPillarShieldHeath)
            {
                NPC.ShieldStrengthTowerVortex = QoLCompendium.mainServerConfig.LunarPillarShieldHeath;
            }

            if (NPC.ShieldStrengthTowerSolar > QoLCompendium.mainServerConfig.LunarPillarShieldHeath)
            {
                NPC.ShieldStrengthTowerSolar = QoLCompendium.mainServerConfig.LunarPillarShieldHeath;
            }

            if (NPC.ShieldStrengthTowerNebula > QoLCompendium.mainServerConfig.LunarPillarShieldHeath)
            {
                NPC.ShieldStrengthTowerNebula = QoLCompendium.mainServerConfig.LunarPillarShieldHeath;
            }

            if (NPC.ShieldStrengthTowerStardust > QoLCompendium.mainServerConfig.LunarPillarShieldHeath)
            {
                NPC.ShieldStrengthTowerStardust = QoLCompendium.mainServerConfig.LunarPillarShieldHeath;
            }
        }
    }
    ...
    public class PillarsDropMore : GlobalNPC
    {
        ...
        {
            if (LunarPillars.Contains(npc.type) && QoLCompendium.mainServerConfig.LunarPillarsDropMoreFragments)
            {
    ...
    public class BestiaryUnlock : GlobalNPC
    {
        ...
        {
            if (QoLCompendium.mainServerConfig.OneKillForBestiaryEntries)
    ...
    public class EnemiesBreakDoors : ModSystem
    {
        public override void Load()
        {
            if (QoLCompendium.mainServerConfig.NoDoorBreaking)
    ...
    public class NoLavaFromSlimes : GlobalNPC
    {
        public override void HitEffect(NPC npc, NPC.HitInfo hit)
        {
            ...
            try
            {
                if (QoLCompendium.mainServerConfig.LavaSlimesDontDropLava)
                {
    ...
    public class NPCDrops : GlobalNPC
    {
        public override void SetDefaults(NPC npc)
        {
            npc.value *= QoLCompendium.mainServerConfig.EnemiesDropMoreCoins;
        }

        public override void ModifyNPCLoot(NPC npc, NPCLoot npcLoot)
        {
            if (npc.type == NPCID.DD2DarkMageT1 && QoLCompendium.mainServerConfig.ExtraDefenderMedalDrops)
            {
                npcLoot.Add(ItemDropRule.Common(ItemID.DefenderMedal, 1, 10, 10));
            }

            if (npc.type == NPCID.DD2DarkMageT3 && QoLCompendium.mainServerConfig.ExtraDefenderMedalDrops)
            {
                npcLoot.Add(ItemDropRule.Common(ItemID.DefenderMedal, 1, 25, 25));
            }

            if (npc.type == NPCID.DD2OgreT2 && QoLCompendium.mainServerConfig.ExtraDefenderMedalDrops)
            {
                npcLoot.Add(ItemDropRule.Common(ItemID.DefenderMedal, 1, 25, 25));
            }

            if (npc.type == NPCID.DD2OgreT3 && QoLCompendium.mainServerConfig.ExtraDefenderMedalDrops)
            {
                npcLoot.Add(ItemDropRule.Common(ItemID.DefenderMedal, 1, 30, 30));
            }

            if (npc.type == NPCID.DD2Betsy && QoLCompendium.mainServerConfig.ExtraDefenderMedalDrops)
            {
                npcLoot.Add(ItemDropRule.Common(ItemID.DefenderMedal, 1, 50, 50));
            }

            if ((npc.type == NPCID.NebulaHeadcrab || npc.type == NPCID.NebulaBeast || npc.type == NPCID.NebulaBrain || npc.type == NPCID.NebulaSoldier) && QoLCompendium.mainServerConfig.LunarPillarsDropMoreFragments)
            {
                npcLoot.Add(ItemDropRule.Common(ItemID.FragmentNebula, 1, 1, 3));
            }

            if ((npc.type == NPCID.SolarCorite || npc.type == NPCID.SolarCrawltipedeHead || npc.type == NPCID.SolarSpearman || npc.type == NPCID.SolarDrakomire || npc.type == NPCID.SolarDrakomireRider || npc.type == NPCID.SolarSolenian || npc.type == NPCID.SolarSroller) && QoLCompendium.mainServerConfig.LunarPillarsDropMoreFragments)
            {
                npcLoot.Add(ItemDropRule.Common(ItemID.FragmentSolar, 1, 1, 3));
            }

            if ((npc.type == NPCID.StardustJellyfishBig || npc.type == NPCID.StardustWormHead || npc.type == NPCID.StardustCellBig || npc.type == NPCID.StardustSoldier || npc.type == NPCID.StardustSpiderBig) && QoLCompendium.mainServerConfig.LunarPillarsDropMoreFragments)
            {
                npcLoot.Add(ItemDropRule.Common(ItemID.FragmentStardust, 1, 1, 3));
            }

            if ((npc.type == NPCID.VortexHornet || npc.type == NPCID.VortexHornetQueen || npc.type == NPCID.VortexRifleman || npc.type == NPCID.VortexSoldier) && QoLCompendium.mainServerConfig.LunarPillarsDropMoreFragments)
            {
    ...
    public class NoTownSlimes : GlobalNPC
    {
        ...
        {
            if (QoLCompendium.mainServerConfig.NoTownSlimes)
            {
    ...
    public class AnglerReset : GlobalNPC
    {
        ...
        {
            if (QoLCompendium.mainServerConfig.AnglerQuestInstantReset && Main.anglerQuestFinished)
            {
