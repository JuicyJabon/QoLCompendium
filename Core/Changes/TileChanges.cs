...
{
    public class ExtractinatorTile : GlobalTile
    {
        public override void MouseOver(int i, int j, int type)
        {
            if (type == TileID.Extractinator || type == TileID.ChlorophyteExtractinator)
            {
                Main.LocalPlayer.GetModPlayer<ExtractinatorPlayer>().fastExtractinatorSpeed = true;
            }
        }
    }

    public class ExtractinatorPlayer : ModPlayer
    {
        public bool fastExtractinatorSpeed = false;

        public override void ResetEffects()
        {
            fastExtractinatorSpeed = false;
        }
    }
    ...
    public class ExtractinatorItem : GlobalItem
    {
        ...
        {
            if (item.createTile > -1 && item.damage == -1)
            {
                if (QoLCompendium.mainServerConfig.FasterExtractinator && player.GetModPlayer<ExtractinatorPlayer>().fastExtractinatorSpeed)
                {
                    item.useTime = 2;
                    item.useAnimation = 3;
                }
    ...
    public class HellstoneSpelunker : GlobalTile
    {
        ...
        {
            if (QoLCompendium.mainServerConfig.HellstoneWorksWithSpelunker && type == TileID.Hellstone)
                return true;
            return base.IsTileSpelunkable(i, j, type);
        }
    }

    public class PurificationPowerWallFix : GlobalProjectile
    {
        public override void AI(Projectile projectile)
        {
            if (projectile.type == ProjectileID.PurificationPowder && QoLCompendium.mainServerConfig.PurificationPowderCleansesWalls)
            {
                Point16 projPosition = (projectile.position + new Vector2(Main.rand.NextFloat(projectile.width), Main.rand.NextFloat(projectile.height))).ToTileCoordinates16();
                ushort[] evilWallIDs = Common.EvilWallIDs;
                ushort[] pureWallIDs = Common.PureWallIDs;
                for (int i = 0; i < evilWallIDs.Length; i++)
                {
                    Tile tile = Main.tile[projPosition];
                    if (tile.WallType == evilWallIDs[i])
                    {
                        tile = Main.tile[projPosition];
                        tile.WallType = pureWallIDs[i];
                    }
                }
            }
        }
    ...
    public class DontBreakHive : ModSystem
    {
        ...
        public override void Load()
        {
            ...
            if (QoLCompendium.mainServerConfig.NoLarvaBreak)
            {
    ...
    public class FastTreeGrowth : GlobalTile
    {
        ...
        {
            if (!QoLCompendium.mainServerConfig.FastTreeGrowth || !Main.tile[i, j].HasTile)
            {
                return;
            }
    ...
    public class FastHerbGrowth : ModSystem
    {
        ...
        private void WorldGen_GrowAlch(ILContext il)
        {
            ...
            if (!c.TryGotoNext(MoveType.After,
                ...
                return;
            c.EmitDelegate<Func<int, int>>(num => QoLCompendium.mainServerConfig.FastHerbGrowth ? 1 : num);
        }
        ...
        {
            return QoLCompendium.mainServerConfig.FastHerbGrowth || orig.Invoke(self, style);
        }

        private bool WorldGen_IsHarvestableHerbWithSeed(On_WorldGen.orig_IsHarvestableHerbWithSeed orig, int type,
            int style)
        {
            return QoLCompendium.mainServerConfig.FastHerbGrowth || orig.Invoke(type, style);
        }
        ...
        private static void Player_PlaceThing_Tiles_BlockPlacementForAssortedThings(ILContext il)
        {
            ...
            c.EmitDelegate(() =>
            {
                if (QoLCompendium.mainServerConfig.RegrowthAutoReplant)
                {
            ...
            c.EmitDelegate<Func<bool, bool>>(flag =>
            {
                ...
                return QoLCompendium.mainServerConfig.FastHerbGrowth || flag;
            });
    ...
    public class TreeDrops : ModSystem
    {
        ...
        {
            if (QoLCompendium.mainServerConfig.TreesDropMoreWhenShook)
            {
                dropItem = gemType;
    ...
    public class NoPylonRestrictions : GlobalPylon
    {
        ...
        {
            if (QoLCompendium.mainServerConfig.NoPylonTeleportRestrictions)
                return true;
            return null;
        }
