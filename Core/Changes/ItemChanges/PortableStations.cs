...
namespace QoLCompendium.Core.Changes.ItemChanges
{
    ...
    {
        public override bool IsLoadingEnabled(Mod mod) => !ModLoader.HasMod("ImproveGame");
        ...
        {
            if (!QoLCompendium.mainConfig.PortableStations)
            {
                orig.Invoke(canDelayCheck);
                return;
            }
            var origGraveyard = Main.LocalPlayer.ZoneGraveyard;
            var origSnow = Main.LocalPlayer.ZoneSnow;

            CheckZoneItemFromPlayer(Main.LocalPlayer);
            orig.Invoke(canDelayCheck);

            Main.LocalPlayer.ZoneGraveyard = origGraveyard;
            Main.LocalPlayer.ZoneSnow = origSnow;
        }
        ...
        internal static void CheckStations(IEnumerable<Item> items)
        {
            foreach (var item in items)
            {
                ...
                if (item.type is ItemID.BottomlessShimmerBucket)
                {
                    Main.LocalPlayer.adjShimmer = true;
                }

                if (QoLCompendium.itemConfig.BottomlessBuckets)
                {
                    if (item.type == ModContent.ItemType<BottomlessLiquidBucket>())
                    {
                        Main.LocalPlayer.adjWater = true;
                        Main.LocalPlayer.adjLava = true;
                        Main.LocalPlayer.adjHoney = true;
