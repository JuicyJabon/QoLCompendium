...
namespace QoLCompendium.Core.UI
{
    public class UIGrid : UIElement
    {
        ...
        {
            BuffButton b1 = item1 as BuffButton;
            BuffButton b2 = item2 as BuffButton;

            if (b1 == null || b2 == null)
                return 0;

            string name1 = Lang.GetBuffName(b1.BuffId);
            string name2 = Lang.GetBuffName(b2.BuffId);
            int nameComparision = String.Compare(name1.ToLower(), name2.ToLower());

            if (b1.EffectType == b2.EffectType)
                return nameComparision;
            else
                return b1.EffectType.CompareTo(b2.EffectType);
        }
