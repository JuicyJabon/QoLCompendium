...
{
    public class Hellbridger : ModItem, ILocalizedModType
    {
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            if (player.altFunctionUse == 2)
                Projectile.NewProjectile(player.GetSource_ItemUse(source.Item), mouse, Vector2.Zero, ModContent.ProjectileType<HellbridgerSingleProj>(), 0, 0, player.whoAmI);
            else
        ...
        public void HandleShadow(Player player)
        {
            ...
            for (int i = -100; i <= 100; i++)
            {
                Vector2 mouse = Main.MouseWorld;
                mouse.X += i * 16;
                Projectile.NewProjectile(player.GetSource_ItemUse(Item), mouse - new Vector2(0, 320), Vector2.Zero, ModContent.ProjectileType<BuildIndicatorProjectile>(), 0, 0f, player.whoAmI);
            }
        }
    }
}
