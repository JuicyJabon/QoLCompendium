namespace QoLCompendium.Content.Projectiles.Dedicated
{
    public abstract class WhipProjectile : ModProjectile
    {
        ...
        public override void SetDefaults()
        {
            ...
            whipSegment = (Texture2D)ModContent.Request<Texture2D>($"{nameof(QoLCompendium)}/Content/Projectiles/Dedicated/SillySlapperWhip_Segment", AssetRequestMode.AsyncLoad);
            whipTip = (Texture2D)ModContent.Request<Texture2D>($"{nameof(QoLCompendium)}/Content/Projectiles/Dedicated/SillySlapperWhip_Tip", AssetRequestMode.AsyncLoad);
            SetWhipStats();
        }
        ...
        internal bool DrawWhip(Color lineColor)
        {
            ...
            for (int i = 0; i < whipPoints.Count - 1; i++)
            {
                ...
                float num6 = vector4.ToRotation() + segmentRotation;
                if (i == 0)
                {
                    num6 = vector4.ToRotation();
                }
